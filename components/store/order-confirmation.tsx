    ...
    <div className="grid gap-8 lg:grid-cols-[1.2fr,0.8fr]">
      <div className="glass-panel space-y-6 p-8">
        <div className="space-y-3">
          ...
          <p className="text-base leading-8 text-black/70">
            Order <span className="font-semibold text-ink">{order.orderNumber}</span> has been paid and saved. A receipt can be sent from Stripe if you enable receipts in your Stripe dashboard.
          </p>
        </div>
        <div className="space-y-4">
          {order.items.map((item) => (
            <div key={item.id} className="flex gap-4 rounded-[1.5rem] border border-black/10 bg-white/70 p-4">
              <img src={item.imageUrl || "/products/placeholder.svg"} alt={item.title} className="h-24 w-20 rounded-2xl border border-black/10 bg-white object-cover" />
      ...
      <aside className="glass-panel h-fit space-y-4 p-8">
        ...
        <div className="grid gap-3">
          ...
          <Link href="/admin/orders" className="text-sm font-medium text-black/55 hover:text-ink">
            View in admin
          </Link>
        </div>
      </aside>
